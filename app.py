...
app.config['MONGO_URI']= 'mongodb://localhost:27017/nft'
...
def uploadResume():
    if 'user_id' in session and 'user_name' in session:
        try:
            ...
            # Save the uploaded resume to the upload folder
            if file and allowedExtension(file.filename):
                file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
                print("Resume Uploaded")

                # Generate the URL for the uploaded resume
                resume_preview = url_for('static', filename='uploaded_resumes/' + filename)
                ...
                if result is None:
                    ...
                    return render_template("EmployeeDashboard.html", successMsg="Resume Screen Successfully!!", resume_preview=resume_preview)
            else:
